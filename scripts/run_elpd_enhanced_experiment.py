#!/usr/bin/env python3
"""
Enhanced Experiment: Test ELPD-Blend with DIFFERENT target/surrogate architectures.

This properly tests the black-and-white-power-likelihood hypothesis:
- Surrogate: ResNet18 (white-box gradient source)
- Target: VGG16 (black-box query-only model)

This setup tests whether adaptive blending beats both pure transfer and pure query attacks.
"""

import os
import torch
import torch.nn.functional as F
from types import SimpleNamespace
from pathlib import Path
import json

from ensemble_attacks.models import get_resnet18, get_vgg16
from ensemble_attacks.data import get_cifar10_loaders
from ensemble_attacks.utils import get_device
from ensemble_attacks.baselines import run_mifgsm, run_nes_only, run_static_hybrid
from ensemble_attacks.elpd_attack import elpd_blend_attack
from ensemble_attacks.elpd_blender import ELPDBlender
from ensemble_attacks.query_estimator import QueryEstimator


# ── Model wrappers ────────────────────────────────────────────────────────────

class SurrogateModel:
    """White-box surrogate model (ResNet18)."""
    def __init__(self, model, device):
        self.model = model
        self.device = device

    def gradient(self, x, true_label):
        """Compute gradient of cross-entropy loss w.r.t. x."""
        x_batch = x.unsqueeze(0).clone().detach().requires_grad_(True)
        logits = self.model(x_batch)
        loss = F.cross_entropy(logits, torch.tensor([true_label], device=self.device))
        self.model.zero_grad()
        loss.backward()
        grad = x_batch.grad.clone().detach().squeeze(0)
        return grad.view(-1)


class TargetModel:
    """Black-box target model (VGG16) - query-only interface."""
    def __init__(self, model, device, query_budget=1000):
        self.model = model
        self.device = device
        self.queries_used = 0
        self.query_budget = query_budget

    def predict(self, x):
        """Predict class for single image (C, H, W)."""
        self.queries_used += 1
        if self.queries_used > self.query_budget:
            raise RuntimeError(f"Query budget exceeded: {self.queries_used} > {self.query_budget}")
        with torch.no_grad():
            logits = self.model(x.unsqueeze(0))
            return logits.argmax(dim=1).item()

    def get_query_fn(self):
        """Return batch query function for NES/SPSA."""
        def query_fn(x_batch):
            """x_batch: (B, C, H, W), returns loss (B,)."""
            self.queries_used += x_batch.shape[0]
            if self.queries_used > self.query_budget:
                raise RuntimeError(f"Query budget exceeded: {self.queries_used} > {self.query_budget}")
            with torch.no_grad():
                logits = self.model(x_batch)
                log_probs = F.log_softmax(logits, dim=1)
                losses = -log_probs.max(dim=1).values
                return losses
        return query_fn

    def reset_queries(self):
        self.queries_used = 0


def get_config():
    """Enhanced config for cross-architecture attack."""
    cfg = SimpleNamespace()
    
    cfg.attack = SimpleNamespace(
        num_steps=25,
        step_size=2 / 255,
        epsilon=8 / 255,
        clip_min=0.0,
        clip_max=1.0,
        early_stop=True,
        query_budget=500  # More constrained budget
    )

    cfg.momentum = SimpleNamespace(mu=0.9, enabled=True)

    cfg.query_estimator = SimpleNamespace(
        method='nes',
        sigma=1.5e-2,
        n_samples=8,
        rao_blackwell=True
    )

    cfg.elpd_blender = SimpleNamespace(
        method='waic',
        waic=SimpleNamespace(min_samples_for_waic=6, llik_temperature=0.5),
        loo_psis=SimpleNamespace(pareto_k_threshold=0.7),
        cosine_var=SimpleNamespace(var_weight=0.1),
        eta_grid=SimpleNamespace(low=0.0, high=1.0, n_points=21),
        eta_min=0.0,
        eta_max=1.0,
        eta_ema_alpha=0.4
    )

    cfg.baselines = SimpleNamespace(
        run_static_hybrid=SimpleNamespace(eta_static=0.5)
    )

    return cfg


def run_attack_on_sample(x, true_label, attack_name, surrogate, target, cfg, device):
    """Run single attack and return metrics."""
    target.reset_queries()

    try:
        if attack_name == "mifgsm":
            result = run_mifgsm(x, true_label, surrogate, target, cfg)
        elif attack_name == "nes_only":
            estimator = QueryEstimator(cfg.query_estimator, target.get_query_fn())
            result = run_nes_only(x, true_label, target, estimator, cfg)
        elif attack_name == "static_hybrid":
            estimator = QueryEstimator(cfg.query_estimator, target.get_query_fn())
            result = run_static_hybrid(x, true_label, surrogate, target, estimator, cfg)
        elif attack_name == "elpd_blend":
            blender = ELPDBlender(cfg.elpd_blender, cfg.query_estimator.sigma)
            estimator = QueryEstimator(cfg.query_estimator, target.get_query_fn())
            result = elpd_blend_attack(x, true_label, surrogate, target, blender, estimator, cfg)
        else:
            raise ValueError(f"Unknown attack: {attack_name}")

        linf = (result.adv_x - x).abs().max().item()
        
        return {
            "success": result.success,
            "queries": result.queries_used,
            "steps": result.steps_taken,
            "linf": linf,
            "eta_history": result.eta_history if hasattr(result, 'eta_history') else [],
        }
    except Exception as e:
        return {
            "success": False,
            "queries": target.queries_used,
            "steps": 0,
            "linf": 0.0,
            "error": str(e)
        }


def main():
    device = get_device()
    print(f"Using device: {device}\n")

    output_dir = Path("outputs/elpd_enhanced_experiment")
    output_dir.mkdir(parents=True, exist_ok=True)

    # Load data
    print("Loading CIFAR-10 test set...")
    _, test_loader = get_cifar10_loaders(batch_size=1, num_workers=0)

    # Load models
    print("Loading ResNet18 (surrogate)...")
    surrogate_net = get_resnet18(num_classes=10)
    surrogate_net.load_state_dict(torch.load("resnet18_cifar10.pt", map_location=device))
    surrogate_net.to(device).eval()

    print("Loading VGG16 (target)...")
    target_net = get_vgg16(num_classes=10)
    target_net.load_state_dict(torch.load("vgg16_cifar10.pt", map_location=device))
    target_net.to(device).eval()

    # Wrap models
    surrogate = SurrogateModel(surrogate_net, device)
    target = TargetModel(target_net, device, query_budget=500)

    cfg = get_config()
    attacks = ["mifgsm", "nes_only", "static_hybrid", "elpd_blend"]
    num_samples = 50

    all_results = {attack: [] for attack in attacks}

    print(f"\nRunning attacks on {num_samples} CIFAR-10 test samples...")
    print("Surrogate: ResNet18 | Target: VGG16 (different architectures)")
    print("-" * 70)

    for batch_idx, (x_batch, y_batch) in enumerate(test_loader):
        if batch_idx >= num_samples:
            break

        x = x_batch[0].to(device, dtype=torch.float32)
        true_label = y_batch[0].item()

        if (batch_idx + 1) % 10 == 0:
            print(f"\n[{batch_idx+1}/{num_samples}]", end="")

        for attack_name in attacks:
            result = run_attack_on_sample(x, true_label, attack_name, surrogate, target, cfg, device)
            all_results[attack_name].append(result)
            status = "✓" if result.get("success") else "✗"
            queries = result.get("queries", 0)
            if (batch_idx + 1) % 10 == 0:
                print(f" {attack_name[:3]}:{status}", end="")

        if (batch_idx + 1) % 10 == 0:
            print()

    # Aggregate results
    print("\n" + "="*70)
    print("EXPERIMENT RESULTS: Cross-Architecture ELPD-Blend")
    print("="*70)

    summary = {}
    for attack_name in attacks:
        results = all_results[attack_name]
        successes = sum(1 for r in results if r.get("success"))
        asr = successes / len(results) if results else 0.0
        avg_queries = sum(r.get("queries", 0) for r in results) / len(results) if results else 0
        avg_linf = sum(r.get("linf", 0) for r in results) / len(results) if results else 0

        summary[attack_name] = {
            "asr": asr,
            "avg_queries": avg_queries,
            "avg_linf": avg_linf,
            "num_samples": len(results)
        }

        print(f"\n{attack_name.upper()}")
        print(f"  ASR (Attack Success Rate): {asr:.1%}")
        print(f"  Avg Queries: {avg_queries:.1f} / {cfg.attack.query_budget}")
        print(f"  Avg L∞ perturbation: {avg_linf:.4f} (ε = {cfg.attack.epsilon:.4f})")

    # Analysis
    print("\n" + "="*70)
    print("ANALYSIS: Black-and-White Power-Likelihood Hypothesis")
    print("="*70)

    mifgsm_asr = summary["mifgsm"]["asr"]
    nes_asr = summary["nes_only"]["asr"]
    static_asr = summary["static_hybrid"]["asr"]
    elpd_asr = summary["elpd_blend"]["asr"]

    print(f"\nTransfer-only (MI-FGSM): {mifgsm_asr:.1%}")
    print(f"Query-only (NES): {nes_asr:.1%}")
    print(f"Fixed blend (Static Hybrid 50/50): {static_asr:.1%}")
    print(f"Adaptive blend (ELPD-Blend): {elpd_asr:.1%}")

    if elpd_asr >= static_asr and elpd_asr >= mifgsm_asr and elpd_asr >= nes_asr:
        print("\n✓ HYPOTHESIS VALIDATED: Adaptive blending meets or exceeds all baselines!")
    else:
        print("\n• Mixed results. Adaptive blending may need tuning for this setup.")

    elpd_queries = summary["elpd_blend"]["avg_queries"]
    static_queries = summary["static_hybrid"]["avg_queries"]
    nes_queries = summary["nes_only"]["avg_queries"]

    print(f"\nQuery efficiency (lower is better):")
    print(f"  ELPD-Blend: {elpd_queries:.1f} queries/sample")
    print(f"  Static Hybrid: {static_queries:.1f} queries/sample")
    print(f"  NES-only: {nes_queries:.1f} queries/sample")

    # Save results
    results_file = output_dir / "results.json"
    with open(results_file, "w") as f:
        json.dump({
            "summary": summary,
            "detailed": all_results,
            "config": {
                "num_samples": num_samples,
                "surrogate": "ResNet18",
                "target": "VGG16",
                "epsilon": float(cfg.attack.epsilon),
                "query_budget": cfg.attack.query_budget,
            }
        }, f, indent=2)

    print(f"\n✓ Results saved to {results_file}")


if __name__ == "__main__":
    main()
